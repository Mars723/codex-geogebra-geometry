...
布局优化器可以在合法坐标范围内搜索，保留题目要求的角、边顺序，同时尽量消除 accidental 关系。默认阻止中、高严重度的未声明关系通过交付门槛。
...
完整 Skill 已被包装为 `geogebra-geometry` Codex 插件，并配有 `codex-geogebra` marketplace 清单。插件和内嵌 Skill 均已通过官方校验器；用户添加本仓库作为 marketplace 后即可安装。
